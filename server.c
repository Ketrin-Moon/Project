...
int main()
{
	...
	struct msgvar var;
	struct for_func_table out_table[HASHTAB_SIZE], sin_table[HASHTAB_SIZE], val_table[HASHTAB_SIZE];
	key_t ipckey;
	pid_t pid;
	...
	pthread_create(&p_out, NULL, (void*)create_table, &out_table);
	strncpy(sin_table->name, "sin_table.txt", 20);
	pthread_create(&p_sin, NULL, (void*)create_table, &sin_table);
	strncpy(val_table->name, "val_table.txt", 20);
	pthread_create(&p_val, NULL, (void*)create_table, &val_table);
